...
interface GenericContentDetailProps {
  item: GenericContent;
  isAdmin: boolean;
  onEdit: () => void;
  onDelete: () => void;
...
const GenericContentDetail: React.FC<GenericContentDetailProps> = ({
  item,
  isAdmin,
  onEdit,
  onDelete,
  ...
  return (
    <div className="bg-[var(--bg-secondary)] p-6 rounded-lg shadow-2xl animate-fade-in">
      ...
      <div className="mb-6 border-b border-[var(--bg-tertiary)] pb-4">
        <h2 className="text-2xl font-bold text-[var(--text-accent)] text-center">{getML(item.title)}</h2>
      </div>
...
export default GenericContentDetail;
